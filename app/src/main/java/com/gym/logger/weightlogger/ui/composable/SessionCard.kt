...
fun SessionCard(
    session: Session,
    onSessionClicked: (session: Session) -> Unit = {},
    onLongClickedClicked: (session: Session) -> Unit = {}
) {
    ...
    Column(modifier = Modifier.fillMaxWidth()) {
        Card(
            ...
            Column(
                ...
                Text(
                    text = AppUtil.getConvertedDate(session.dateCreated),
                    modifier = Modifier
                        .fillMaxWidth()
                        .padding(3.dp),
                    fontSize = 12.sp,
...
fun SessionCardPreview() {
    SessionCard(session = Session(1, "Session1", Calendar.getInstance().time))
}